# TA-P15-006 Console 客户端质量体系与发布门禁冻结（2026-03-03）
...
| 层级 | 目标 | 当前基线命令 | 判定标准 |
| --- | --- | --- | --- |
| L0 Unit | 组件/状态机纯逻辑正确性 | `corepack pnpm --filter @telagent/console test` | 关键模块用例通过率 `100%` |
...
1. `Pre-Merge`：Console build + Node contract tests + SDK TS tests。
...
1. Console build：PASS（日志见 `2026-03-03-p15-console-build-ta-p15-006.txt`）。
2. Console test：当前包返回 `no tests for console package`，记录为已识别测试覆盖缺口。
...
1. 在 `TA-P15-007` Gate 收口中明确 console E2E 最小集是否作为放行前置条件。
...
- 任务文档：`docs/implementation/phase-15/ta-p15-006-console-quality-gates-and-release-readiness-2026-03-03.md`
- Console build：`docs/implementation/phase-15/logs/2026-03-03-p15-console-build-ta-p15-006.txt`
- Console test：`docs/implementation/phase-15/logs/2026-03-03-p15-console-test-ta-p15-006.txt`
