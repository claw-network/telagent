...
- Configure `$TELAGENT_HOME/.env` environment variables (signing keys, chain, ClawNet, etc.)
...
This clones the repo, installs dependencies, generates a private key and passphrase, creates `$TELAGENT_HOME/.env` (default: `~/.telagent/.env`), builds workspace packages, and installs a background service. After completion the node should already be running; use `pnpm dev` only when you want a foreground local-dev process from the repo checkout.
...
| File | Purpose |
|------|---------|
| `skills/telagent/install-and-configure.md` | Node installation, `$TELAGENT_HOME/.env` configuration, key generation, startup |
