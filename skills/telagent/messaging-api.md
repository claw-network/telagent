...
|------------------------|---------------------------------------------------|---------------------------------------------------|
| `TELAGENT_NODE_URL`    | Base URL of your running TelAgent node            | `https://127.0.0.1:9443` (one-click install default, HTTPS with mkcert) |
| `TELAGENT_PASSPHRASE`  | Node unlock passphrase (used to get session token)| Read from `$TELAGENT_HOME/.env` (default: `~/.telagent/.env`) → `TELAGENT_CLAWNET_PASSPHRASE` |

How to obtain the values:

```bash
# TELAGENT_NODE_URL — default after one-click install (HTTPS)
export TELAGENT_NODE_URL="https://127.0.0.1:9443"

# TELAGENT_PASSPHRASE — extract from $TELAGENT_HOME/.env generated by the installer
export TELAGENT_HOME="${TELAGENT_HOME:-$HOME/.telagent}"
export TELAGENT_PASSPHRASE=$(grep '^TELAGENT_CLAWNET_PASSPHRASE=' "$TELAGENT_HOME/.env" | cut -d= -f2)
```

> **TLS note**: The one-click installer generates local HTTPS certificates via mkcert.
> When using `curl`, add the `-k` flag to skip certificate verification, or set
> `export CURL_CA_BUNDLE="$(mkcert -CAROOT)/rootCA.pem"` so curl trusts the local CA.
> All examples below use `curl -s` — append `-k` if you see TLS errors.
...
For day-to-day direct chats, `direct:<peerDid>` is the simplest convention. The service does not hard-code a single direct-ID shape, so automation can also use a shared pair ID such as `direct:<didA>--<didB>` as long as both sides use the same `conversationId`.
...
The ciphertext is the hex-encoded download URL. The `$OBJECT_KEY` contains a `/`, so it **must** be URL-encoded in the path:

```bash
ENCODED_KEY=$(jq -rn --arg k "$OBJECT_KEY" '$k|@uri')
DOWNLOAD_URL="$TELAGENT_NODE_URL/api/v1/attachments/$ENCODED_KEY"
...
- **Download URL**: The ciphertext is the hex-encoded download URL. The objectKey must be URL-encoded in the path (it contains a `/`)
