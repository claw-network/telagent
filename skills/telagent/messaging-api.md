...
| `TELAGENT_PASSPHRASE`  | Node unlock passphrase (used to get session token)| Read from `<install dir>/.env` (default: `~/telagent/.env`) → `TELAGENT_CLAWNET_PASSPHRASE` |
...
export TELAGENT_INSTALL_DIR="${TELAGENT_INSTALL_DIR:-$HOME/telagent}"
export TELAGENT_PASSPHRASE=$(grep '^TELAGENT_CLAWNET_PASSPHRASE=' "$TELAGENT_INSTALL_DIR/.env" | cut -d= -f2)
```
